...
def test_misscat():
    ...
    # Tests if an exception has been raised when a non dataframe is passed to the function
    with pytest.raises(Exception):
        misscat.misscat(df = 1, threshold = 0.3)
        
    # Tests if an exception has been raised when a threshold higher than 1 is passed
    with pytest.raises(Exception):
        misscat.misscat(df = data, threshold = 2)

    # Tests if an exception has been raised when a threshold less than 0 is passed
    with pytest.raises(Exception):
        misscat.misscat(df = data, threshold = -0.1)
